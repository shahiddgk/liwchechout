...
class CommerceConfig
{
    ...
    public function applyConfig()
    {
        ...
        add_filter('woocommerce_get_price', array($this, 'wooClubProductsPrice'),99,2);
        ...
        add_action('woocommerce_cart_calculate_fees', array($this, 'wooAddShippingCostToCart'));
...
//        add_action( 'woocommerce_checkout_process' , array($this, 'check_service_agreement') );
    ...
    */
    ...
    public function stockOutProducts(){
        ...
        if( $product->is_type( 'variable' )) {
            ...
            if ( ! empty( $variables ) ){
                ...
                }elseif(is_user_logged_in() && $user["club_term_meta"]["default_club"] && array_search('default-club',$arr) === FALSE ){
                        echo '<script type="text/javascript">
                                jQuery(document).ready(function($){
                                    $(".products .post-'.$post->ID.'").remove();
                                });
                              </script>';
                }elseif(is_user_logged_in() && ! $user["club_term_meta"]["default_club"] && array_search('private-club',$arr) === FALSE && ! $is_closed_club){
                        echo '<script type="text/javascript">
                                jQuery(document).ready(function($){
                                    $(".products .post-'.$post->ID.'").remove();
                                });
    ...
    public function wooHideVariationClubType()
    {
        ...
        elseif ((is_user_logged_in() && $user["club_term_meta"]["default_club"] === NULL)) {
            $value = 'private-club';
        } else {
            $value = 'default-club';
        }  ?>
        <script type="text/javascript">
                ...
                jQuery(document).ready(function($) {
                    ...
                    $('#pa_club-type').one('change',function(){
                        $('#pa_club-type option[value="'+value+'"]').prop('selected', true);
                    });
                    $('#pa_club-type').trigger('change');
    ...
    /**
     * Add shipping cost to cart by user club or default club
     */
    function wooAddShippingCostToCart()
    {
        $is_closed_club = \LW\Settings::is_closed_club();
        ...
        if (is_user_logged_in() && !current_user_can('administrator')) {
            $shipping_price = (float)$user["club_term_meta"]["clubs_ship_price"];
        } else {
            ...
            foreach ($dclub as $k => $slug) {
                if (get_term_meta($k, 'default_club', true)) {
                    ...
                    $shipping_price = (float)$default["clubs_ship_price"][0];
                }
            }
        }
        $shipping = $woocommerce->cart->cart_contents_weight * $shipping_price;
        if(! $is_closed_club){
            $woocommerce->cart->add_fee(__('Shipping Cost', 'woocommerce'), $shipping);
        }
    }
    ...
    public function formatVariationPriceForProduct($price, $product)
    {
	    ...
	    if( ! $is_closed_club && ! empty( $user["user_info"] ) ){
		    if( $user["club_term_meta"]["chefs_active"] === '1' ){
			    $club_id = $user["user_meta"]["user_club"];
			    $chefs_percent = get_post_meta($product->id,'chefs_club_id_'.$club_id,true);

                if( $product->children ){
	                $var_total = $this->formatVariationPrice($product->children["visible"],$chefs_percent,$club_id,$user);
                }else{
	    ...
	    {
		    if ( ( ! $is_closed_club && is_user_logged_in() && $user["club_term_meta"]["default_club"] === NULL )
    ...
    public function filterPriceForVariableProducts($data, $product, $variation)
    {
        ...
        if( $is_closed_club ){
            ...
            $percents = $default["clubs_discount_p"][0];
        }
        elseif($user["club_term_meta"]["clubs_discount_p"]){
        ...
        if (is_user_logged_in()) {
            if ($user["club_term_meta"]["clubs_discount_p"]) {
                ...
                $data['display_price'] * $percents / 100;
                $data['display_regular_price'] * $percents / 100;
                $data['price_html'] = '<p class="price"><span class="woocommerce-Price-amount amount">' . wc_price($data['display_price']) . '</span></p>';
    ...
    public function addClubConfirmMessageToEmail($order, $sent_to_admin)
    {
        ...
        if($is_closed_club){
            ...
            foreach ($dclub as $k => $val) {
                if ( get_term_meta($val->term_id, 'default_club', true) )
                {
                    $club_term_meta = get_term_meta($val->term_id);
                    $conf_message = $club_term_meta["clubs_confirmation_massage"][0];
                }
            }
        }
    ...
    public function confirmationMessage($order_id)
    {
        ...
        $conf_message = '';
        if(! $is_closed_club ){
            $message = \LW\Settings::currentUser();
            $conf_message = $message['club_term_meta']["clubs_confirmation_massage"];
        }else{
            $dclub = get_terms('clubs', array('fields' => 'all'));
    ...
    {
        $message = \LW\Settings::currentUser();
        echo '<p class="lwu-checkout-message">' . $message['club_term_meta']["clubs_checkout_massage"] . '</p>';
    }
    ...
    public function addOrderCustomFiled($order_id)
    {
        ...
        foreach ($items as $item) {
	        ...
	        if( $club["club_term_meta"]["chefs_active"] === '1' &&  ! $is_closed_club ) {
	            $chefs_individual_percent = get_post_meta( $item["variation_id"], 'chef_var_percent_' . $chefs_id, true );
		        $chef_default_percent = get_post_meta( $item["product_id"], 'chefs_club_id_' . $chefs_id, true );

		        if ( $item["variation_id"] !== '' ) {
		            $prc = ($chefs_individual_percent !== '' ) ? $chefs_individual_percent : $chef_default_percent;
			        update_post_meta( $order_id, 'order_variation_chef_prc_' . $item["variation_id"], $prc );
		        } else {
			        update_post_meta( $order_id, 'order_product_chef_prc_' . $item["product_id"], $chef_default_percent );
		        }
	        }
            ...
            if ($i !== 1) {
                ...
                if (!add_post_meta($order_id, 'clubs-name', $club_name, true)) {
                    update_post_meta($order_id, 'clubs-name', $club_name, true);
                }
                if (!add_post_meta($order_id, 'clubs-slug', $club_slug, true)) {
                    update_post_meta($order_id, 'clubs-slug', $club_slug, true);
                }
                if (!add_post_meta($order_id, 'club-manager', $userID, true)) {
