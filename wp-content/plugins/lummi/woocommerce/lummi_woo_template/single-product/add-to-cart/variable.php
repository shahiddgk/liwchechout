							    ...
							    if ( is_user_logged_in() && ! $is_closed_club && ! $user["club_term_meta"]["default_club"]) {
	                                    foreach ($variations as $k => $value){
		                                    if( $value["attributes"]["attribute_pa_club-type"] == 'private-club'){
			                                    $opt[$k] = $value["attributes"]['attribute_' . sanitize_title( $attribute_name )];
			                                    $selected = $variations[$k]["attributes"]['attribute_' . sanitize_title( $attribute_name )];
		                                    }else{
			                                     unset($opt[$k]);
                                            }
	                                    }
									...
									foreach ($variations as $k => $value){
										if( $value["attributes"]["attribute_pa_club-type"] == 'default-club'){
											$opt[$k] = $variations[$k]["attributes"]['attribute_' . sanitize_title( $attribute_name )];
											$selected = $variations[$k]["attributes"]['attribute_' . sanitize_title( $attribute_name )];
										}else{
											unset($opt[$k]);
										}
