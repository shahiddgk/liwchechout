...
function gtm4wp_add_basic_datalayer_data( $dataLayer ) {
	...
	if ( $gtm4wp_options[ GTM4WP_OPTION_INCLUDE_USERCLUB ] ) {
		$current_user = wp_get_current_user();

		if ( $gtm4wp_options[ GTM4WP_OPTION_INCLUDE_USERCLUB ] && ! current_user_can('administrator') ) {
			$club_id = get_user_meta($current_user->ID)["user_club"][0];
			$club = get_term($club_id);

			$dataLayer["visitorClub"] = ( empty( $club->name ) ? "visitor-logged-out" : $club->name );
		}
