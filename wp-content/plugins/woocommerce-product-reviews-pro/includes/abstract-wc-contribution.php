...
abstract class WC_Contribution {
	...
	/**
	 * Checking if review is editable or not.
	 *
	 * @since 1.8.0
	 * @return bool true if review is editable, false otherwise.
	 */
	public function is_editable() {

		$is_editable = false;

		// checking if user is logged in and review update confirmation email is enabled
		if ( is_user_logged_in() && wc_product_reviews_pro_review_update_confirmation_enabled() ) {
			...
			// checking if comment type is review and if user is admin or review author
			if ( 'review' === $comment->comment_type && ( current_user_can( 'manage_options' ) || (int) $user_id === (int) $comment->user_id ) ) {
				$is_editable = true;
			}
		}
