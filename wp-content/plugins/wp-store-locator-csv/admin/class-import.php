...
if ( !class_exists( 'WPSL_CSV_Import' ) ) {
	...
	class WPSL_CSV_Import {
        ...
        function __construct() {
            ...
            $this->parse_csv = new ParseCsv\Csv();
        ...
        public function check_required_fields( $store_location ) {
            ...
            $store_location = array_change_key_case( $store_location, CASE_LOWER );
        ...
        public function handle_errors() {
            ...
            if ( $error_type == 'upload' ) {
                switch( $error_code ) {
                    case 0:
                        $error = '<p>' . sprintf( __( 'Could not move the uploaded CSV file to the %s folder. Please make sure the folder is writable! %s If your not familiar with making server folders writable, then please contact your host and ask them to assist with it.', 'wpsl-csv' ), '<code>' . WPSL_CSV_IMPORT_DIR . '</code>', '<br><br>' ) . '</p>';
                        break;
                    case 1:
